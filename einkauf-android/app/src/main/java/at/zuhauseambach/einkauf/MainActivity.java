...
public class MainActivity extends Activity {
  ...
  private final String[] brands={"Backbox","Zurück zum Ursprung Bio-Toast","Milsani Irische Butter","Darbo","Natur aktiv","Ich bin Österreich","Ich bin Österreich Bratenaufschnitt","Schärdinger Mondseer","Zurück zum Ursprung","GAZI Ciftlik Naturjoghurt","Schärdinger Formil Vollmilch","Happy Day","Tchibo Feine Milde","Westminster","Obstangebot","Ich bin Österreich Rispentomaten","BILLA immer gut","Günstigste verifizierte Aktionsmarke"};
  private final String[] defaultShop={"HOFER","HOFER","HOFER","SPAR","HOFER","PENNY","PENNY","PENNY","HOFER","PENNY","PENNY","PENNY","PENNY","HOFER","HOFER","PENNY","BILLA","SPAR"};
  private final int[] qty={20,1,2,1,1,2,2,1,12,1,2,2,1,1,2,1,8,10};
  private final String[] currentPrices={"0","1.49","1.69","0","0","0","2.99","4.49","0","1.89","0.89","0","7.99","0","0","2.39","0.55","0"};
  ...
  private void render(){list.removeAllViews();int persons=val(guests,2)+val(hosts,2),d=val(days,2);double factor=(persons*d)/8.0,total=0;
    for(String shop:shops){TextView h=tv(shop,20,true);h.setBackgroundColor(0xffe8efe9);list.addView(h);for(int i=0;i<items.length;i++)if(prefs.getString("s"+i,defaultShop[i]).equals(shop)){LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.VERTICAL);CheckBox c=new CheckBox(this);c.setText(items[i]+" – "+prefs.getString("b"+i,brands[i]));r.addView(c);LinearLayout ed=new LinearLayout(this);ed.setOrientation(LinearLayout.HORIZONTAL);EditText q=number("q"+i,String.valueOf(Math.max(1,(int)Math.ceil(qty[i]*factor))));EditText br=new EditText(this);br.setText(prefs.getString("b"+i,brands[i]));EditText pr=number("p"+i,currentPrices[i]);for(EditText e:new EditText[]{q,br,pr}){e.setLayoutParams(new LinearLayout.LayoutParams(0,-2,1));ed.addView(e);}r.addView(ed);r.addView(tv("Menge | Marke/Hersteller | Preis €",11,false));list.addView(r);try{total+=Double.parseDouble(pr.getText().toString().replace(',','.'));}catch(Exception ignored){}int idx=i;q.setOnFocusChangeListener((v,f)->{if(!f)prefs.edit().putString("q"+idx,q.getText().toString()).apply();});br.setOnFocusChangeListener((v,f)->{if(!f)prefs.edit().putString("b"+idx,br.getText().toString()).apply();});pr.setOnFocusChangeListener((v,f)->{if(!f)prefs.edit().putString("p"+idx,pr.getText().toString()).apply();});}}
    double revenue=val(guests,2)*val(days,2)*val(price,10);sum.setText(String.format(Locale.GERMANY,"Frühstücksumsatz: %.2f € · Kosten: %.2f € · Differenz: %.2f €",revenue,total,revenue-total));
  }
}
