...
public class MainActivity extends Activity {
  ...
  private void build(){ScrollView sc=new ScrollView(this);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(18,18,18,30);sc.addView(root);
    root.addView(tv("Einkauf Zuhause am Bach",26,true));
    root.addView(tv("Preise geprüft für Montag, 3. August 2026 · 0,00 € = noch nicht verifiziert",13,false));
    root.addView(tv("Zwei-Tage-Planung · HOFER → PENNY → BILLA → SPAR",15,false));
    ...
    Button calc=new Button(this);calc.setText("Einkaufsliste berechnen / Preise übernehmen");root.addView(calc);TextView sum=tv("",16,true);root.addView(sum);list=new LinearLayout(this);list.setOrientation(LinearLayout.VERTICAL);root.addView(list);
